...
def load_agent_logs_df(path: str) -> DataFrame:
    ...
    action_cols = [
        "interaction.response.Resolved Action",   # Engine-resolved action (highest priority)
    ...
    # Replace NaN with readable defaults to prevent "nan" appearing in displays
    df["action"] = df["action"].fillna("[NO ACTION — LLM failed to produce a valid action]")
