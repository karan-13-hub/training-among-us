...
class Task:
    def __init__(self, name, duration, task_type, location, map_graph, visual=False):
        self.name = name
        self.max_duration = duration
        self.duration = duration
        self.location = location
        self.task_type = task_type
        self.visual = visual    # Visual tasks produce visible proof (e.g. MedBay Scan)
...
class TaskAssignment:
            ...
            self.tasks.extend(
                [
                    Task(
                        ...
                        room,
                        map_graph,
                        visual=task_config[task_name].get("visual", False),
                    )
    ...
    def assign_tasks_to_players(self, players):
        ...
        for i, player in enumerate(players):
            ...
            # Shuffle tasks so players don't all start with the same task
    ...
    def check_task_completion(self):
        ...
        for task in self.assigned_tasks:
            # Count ALL crewmate tasks (living + ghost) -- ghosts can complete tasks
            # to help the team win, matching standard Among Us rules
            all_tasks += 1
